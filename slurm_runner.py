...
# Added argument to limit concurrent execution
parser.add_argument('-m', '--max-concurrent', type=int, default=None, help="Maximum number of concurrent tasks/nodes.")
...
with open(out_command_file, 'w') as f:
    for i in range(1, n + 1):
        ...
        if not output_dir_in_args:
            ...
            output_dir += f"{i:02d}"
            arguments.append(
                "++output_dir=" + output_dir)  # This will override the output present in the yaml file with the one with the tasknumber
...
if not args[0].dry_run:
    # Formatting array string to conditionally include maximum concurrent limit
    array_param = f"1-{n}"
    if args[0].max_concurrent is not None:
        array_param += f"%{args[0].max_concurrent}"

    full_command = f"export GROUP_SIZE=1; sbatch -J {task_name} -a {array_param} -e logs/slurm-%A_%a.err -o logs/slurm-%A_%a.out  {options_string} {slurm_runner_path}/wrapper_slurm_array_job_group.sh {out_command_file}"
